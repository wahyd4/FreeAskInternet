...
def search_web_ref(query: str, debug=False):
    ...
    content_list = []
    searxng_host = os.environ.get("SEARXNG_HOST", "http://searxng:8080")
    try:

        safe_string = urllib.parse.quote_plus(":all !general " + query)
        response = requests.get(f"{searxng_host}?q={safe_string}&format=json")
        ...
        if search_results.get("results"):
            for item in search_results.get("results")[0:9]:
                ...
                if url:
                    ...
                    icon_url = url_parsed.scheme + "://" + domain + "/favicon.ico"
...
def chat(prompt, stream=True, debug=False):
    openai_host = os.environ.get("OPENAI_HOST", "http://freegpt35:3040")
    model_name = os.environ.get("MODEL_NAME", "gpt-3.5-turbo")

    openai.base_url = f"{openai_host}/v1/"
